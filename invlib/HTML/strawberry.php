...
<!-- (B) BILL TO, SHIP TO, INFO -->
<table id="billship"><tr>
	<td>
		<strong class="pink">BILL TO</strong><br>
		<?php foreach ($this->billto as $b) { echo "$b<br>"; } ?>
	</td>
	<td>
		<strong class="pink">SHIP TO</strong><br>
		<?php foreach ($this->shipto as $s) { echo "$s<br>"; } ?>
	</td>
	<td><?php
		foreach ($this->head as $i) {	echo "<strong class='pink'>{$i[0]}:</strong> {$i[1]}<br>"; }
	?></td>
</tr></table>
